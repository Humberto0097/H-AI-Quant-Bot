...
with tab_stats:
    ...
    st.markdown("""<details class="cyber-manual"><summary>MANUAL DE OPERACIÓN: STATS RADAR</summary><p><b>Función:</b> Análisis profundo de mercados secundarios: corners, tarjetas, tiros, rebotes y puntos por equipo.<br><b>Uso:</b> Selecciona el deporte y el objetivo. La IA buscará promedios históricos y tendencias recientes para darte el Radar de Probabilidad.</p></details>""", unsafe_allow_html=True)
    
    colS1, colS2 = st.columns([1, 2])
    with colS1:
        tipo_stats = st.radio("Módulo de Análisis:", ["Fútbol (Tiros/Corners/Cards)", "Basket (Rebotes/Puntos Team)"], key="radio_stats_web")
        st.info("💡 Este módulo ignora el xG general y se enfoca en el comportamiento volátil del juego.")
    
    with colS2:
        input_stats = st.text_area("📦 Datos/Equipos a Analizar (Stats):", placeholder="Ej: Real Madrid vs City - Analizar corners y tarjetas...", key="area_stats_web")
        
        if st.button("📡 Escanear Micro-Tendencias", key="btn_stats_web"):
            if not input_stats:
        ...
        if 'res_stats_radar_web' in st.session_state:
            st.markdown(st.session_state['res_stats_radar_web'], unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
